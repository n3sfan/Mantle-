...
public class NBTTagList extends NBTBase implements java.lang.Iterable<NBTBase> {
	...
	public void appendTag(NBTBase nbt) {
		...
		if (nbt.getId() == 0) {
			logger.warning("Invalid TagEnd added to ListTag");
		} else {
			if (this.tagType == 0) {
				this.tagType = nbt.getId();
			} else if (this.tagType != nbt.getId()) {
				logger.warning("Adding mismatching tag types to tag list");
				return;
			}
	...
	public void set(int idx, NBTBase nbt) {
		...
		if (nbt.getId() == 0) {
			...
			logger.warning("index out of bounds to set tag in tag list");
		}
	}
